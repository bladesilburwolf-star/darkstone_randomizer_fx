...
The **PC randomizer is active** (v1.4.0-pc). Campaign `DATA.MTF` can be rebuilt safely after fixing shared-offset TOC aliases. Prefer same-size PCLASS TXT + DAT patches; optional one-click inject into DATA.MTF.

PSX remains the size-preserving PSM path; PC uses MTF rebuild + loose PCLASS/DAT.
