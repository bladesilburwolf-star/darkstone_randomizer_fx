...
if not exist "out-pc\com\serifsystemworks\darkstone\MtfExplorerApp.class" (
...
if not exist "%JFX_LIB%\javafx.controls.jar" (
    ...
    echo     Edit JFX_HOME in run-mtf.bat / build-pc.bat
    exit /b 1
)

echo Launching Darkstone MTF Explorer...
java -Xmx2G ^
  --module-path "%JFX_LIB%" --add-modules javafx.controls,javafx.graphics ^
  -Djava.library.path="%JFX_BIN%" ^
  -cp out-pc ^
  com.serifsystemworks.darkstone.MtfExplorerApp %*
endlocal
